...
#epoch <= 200
            ...
            for items in train_loader:
                ...
                # inpaint with edge model
                    ...
                    if True or np.random.binomial(1, 0.5) > 0:
                        ...
                        outputs = outputs * (1-masks) + edges * (masks)
                    else:
                        outputs = edges
                ...
                # joint model
                    ...
                    psnr = self.psnr(self.postprocess(images), self.postprocess(outputs_merged))
                    mae = (torch.sum(torch.abs(images - outputs_merged)) / torch.sum(images)).float()
                    precision, recall = self.edgeacc(edges * (1-masks), e_outputs * (1-masks))
    ...
    def test(self):
        ...
        for items in test_loader:
            ...
            # inpaint with edge model / joint model
                ...
                e_outputs = self.edge_model(images_gray, edges, masks).detach()
                edges_out = e_outputs * (1-masks) + edges * masks
                outputs = self.inpaint_model(images, edges_out, masks)
                outputs_merged = (outputs * (1-masks)) + (images * (masks))
            ...
            if self.debug:
                ...
                edges_out = self.postprocess(1 - edges_out)[0]
                masked = self.postprocess((1-images) * (1-masks) + images)[0]
                masked_gray = self.postprocess((1-images_gray) * (1-masks) + images_gray)[0]
                fname, fext = name.split('.')

                imsave(masked_gray, os.path.join(self.results_path, fname + '_gray.' + fext)) 
                imsave(edges, os.path.join(self.results_path, fname + '_edge.' + fext))
                imsave(edges_out, os.path.join(self.results_path, fname + '_outedge.' + fext))
                imsave(masks*255, os.path.join(self.results_path, fname + '_mask.' + fext))
    ...
    def sample(self, it=None):
        ...
        if model == 1:
            iteration = self.edge_model.iteration
            inputs = ((1-images_gray) * (1-masks))+images_gray
            outputs = self.edge_model(images_gray, edges, masks)
            outputs_merged = (outputs * (1-masks)) + (edges * (masks))
        ...
        # inpaint model
            ...
            iteration = self.inpaint_model.iteration
            inputs = (1-images) * (1-masks) + images
            outputs = self.inpaint_model(images, edges, masks)
            outputs_merged = (outputs * (1-masks)) + (images * (masks))
